...
set pages 700 lines 700 timing on long 9999999 numwidth 20 heading on echo on verify on feedback off serverout on wrap off colsep '|'
prompt ##############################################################
...
prompt ##############################################################
select INSTANCE_NAME, STATUS, DATABASE_STATUS, ACTIVE_STATE, to_char(STARTUP_TIME, 'YYYY-MM-DD hh24:mi:ss') from gv$instance;
prompt ##############################################################
...
prompt ##############################################################
Select count(*) "No. of Archive Logs generated" from v$log_history  where to_char(first_time,'YYY-MM-DD hh24:mi:ss') in (to_char(sysdate-1,'YYY-MM-DD hh24:mi:ss'));
prompt ##############################################################
...
col status for a10
select patch_id, status, Action,Action_time from dba_registry_sqlpatch order by action_time;
prompt ##############################################################
