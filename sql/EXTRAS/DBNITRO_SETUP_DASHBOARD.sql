...
--      , 10 * SUM(CASE WHEN wait_class ='Other'          THEN 1 ELSE 0 END) "Other"
    ...
    FROM (SELECT a.*
           ...
           , NVL(event, session_state)||
              CASE WHEN event like 'enq%' AND session_state = 'WAITING'
                THEN ' [mode='||BITAND(p1, POWER(2,14)-1)||']'
                THEN a.event IN (SELECT name FROM v$event_name WHERE parameter3 = 'class#') 
              ...
              END event2 
           , CASE WHEN a.session_type = 'BACKGROUND' OR REGEXP_LIKE(a.program, '.*\([PJ]\d+\)') THEN REGEXP_REPLACE(SUBSTR(a.program,INSTR(a.program,'(')), '\d', 'n')
      ...
      , dba_users u
      , (SELECT object_id,data_object_id,owner,object_name,subobject_name,object_type
