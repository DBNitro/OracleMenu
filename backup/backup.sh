...
SoftwareVersion="3.2.21"
DateCreation="25/01/2011"
DateModification="12/07/2024"
...
#
DB_SIZEG="$(echo "select trim(to_char(sum(bytes)/1024/1024/1024,'9G999G999D999')) from (select sum(bytes) bytes from dba_data_files union all select sum(bytes) bytes from dba_temp_files union all select sum(bytes * members) from v\$log union all select sum(block_size * file_size_blks) from v\$controlfile);" | sqlplus -S / as sysdba | tail -2)"
#
DB_SIZET="$(echo "select trim(to_char(sum(bytes)/1024/1024/1024/1024,'9G999G999D999')) from (select sum(bytes) bytes from dba_data_files union all select sum(bytes) bytes from dba_temp_files union all select sum(bytes * members) from v\$log union all select sum(block_size * file_size_blks) from v\$controlfile);" | sqlplus -S / as sysdba | tail -2)"
#
# "-------------------------------------------------------------------------------------------------------------"
#
# "-------------------------------------------------------------------------------------------------------------"
...
# sed -i -e 's/\r$//' /opt/dbnitro/backup/backup.sh
...
backup recovery files to destination '/u01/app/oracle/backup';
